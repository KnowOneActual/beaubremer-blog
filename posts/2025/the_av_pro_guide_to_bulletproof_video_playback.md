...
Picture this: It is 30 minutes before showtime. You are handed a video file that needs to be added to the show
immediately. When testing, it plays for eight seconds, then stutters and freezes. You panic.
...
The trick is that you can put H.264 inside an `.mp4` box _or_ a `.mov` box. It is the same video in a different box. But
...
Every couple of seconds, you need a full frame (keyframe). This lets the playback software jump around without decoding
the whole file.
...
If the input is 23.976, 25, or 29.97 fps, convert it to match your QLab project or change the project setting (usually
24 fps):
